...
Apple Bridge exposes 7 macOS domains through 37 MCP tools:
...
| **Contacts** (6) | `contacts_search`, `contacts_get`, `contacts_me`, `contacts_open`, `contacts_create`, `contacts_update` | AppleScript |
...
- Swift 6.2+ to build from source (CI builds and tests on 6.2.0 and the newest hosted toolchain);
  the pre-built release binary has no toolchain requirement
...
The executable will be at `.build/release/apple-bridge`. That path is regenerated by every build,
so for day-to-day use install a signed copy to a stable location instead:

```bash
./scripts/install.sh                 # builds, signs, installs to ~/bin/apple-bridge
./scripts/install.sh --prefix DIR    # installs to DIR/apple-bridge
```

Pre-built Apple Silicon binaries are attached to
[GitHub Releases](https://github.com/boutquin/apple-bridge/releases) with a SHA-256 checksum
(`shasum -a 256 -c apple-bridge-<version>-macos-arm64.tar.gz.sha256`). Each release's notes state
how that binary was signed.
...
└── scripts/
    ├── install.sh          # Build, sign, and install to a stable path
...
| Contacts | AppleScript | AppleScript | `AppleScriptContactsAdapter` |
...
func searchNotes() async throws {
    ...
    #expect(results.items[0].title == "Meeting Notes")
}
```
...
Apple Bridge is built on the official [MCP Swift SDK](https://github.com/modelcontextprotocol/swift-sdk)
(0.12.x), which negotiates protocol versions 2024-11-05 through 2025-11-25:
...
3. Enable access for your terminal application or MCP host app (e.g. Claude). For Calendars, choose
   **Full Access** — "Add Events Only" allows creates but fails every read.
...
2. Enable the app that launches `apple-bridge` — your MCP host (e.g. Claude) or, when running it
   directly, your terminal. macOS attributes the request to that responsible parent process; see
   *Granting permissions when launched by an MCP host* above.
3. Restart the bridge (`pkill -f apple-bridge`) so the new grant is picked up
...
- `mail_search` on a very large mailbox can exceed the MCP host's tool timeout — see
  *Known limitation — AppleScript `whose` searches on large stores* above
- Prefer specific search terms; `mail_unread` is unaffected
